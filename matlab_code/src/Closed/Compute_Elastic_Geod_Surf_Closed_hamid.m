function [F2n,F1,F2nb,H,Hstart,gamcum] = Compute_Elastic_Geod_Surf_Closed_hamid(F1,F2,N,Theta,Phi,Psi,b,gridrotval,eps)
...
while run==1
    run=0;
    try
        [F2n,H1,gamcum,iter1,A] = ReParamclosed(f1,f2new,A_tmp21,A_tmp22new,b,gamcum,gamid,Theta,Phi,Psi,2,A,winsize,79,eps);
    catch
        if eps<epsmin
            fprintf('Giving Up')
            gamcum=gamid;
...
while run==1
    run=0;
    try
        [F2n,H2,gamcum,iter1,A] = ReParamclosed(f1,F2n,sqrtmultfact1,sqrtmultfact2n,b,gamcum,gamid,Theta,Phi,Psi,iter1+1,A,winsize,75,eps);
    catch
        if eps<epsmin
            fprintf('Giving Up')
%             gamcum=gamid;
...
while run==1
    run=0;
    try
        [F2n,H3,gamcum,iter1,A] = ReParamclosed(f1,F2n,sqrtmultfact1,sqrtmultfact2n,b,gamcum,gamid,Theta,Phi,Psi,iter1+1,A,winsize,500,eps);
    catch
        if eps<epsmin
            fprintf('Giving Up')
%             gamcum=gamid;
